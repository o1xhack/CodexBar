...
enum CostUsagePricing {
    ...
    ///
    /// History:
    /// - `14` (0.56.0.1): merged upstream v0.54.1-v0.56.0 Codex scanner semantics,
    ///   including maximum cache-token selection, stale-regression handling,
    ///   omitted reasoning fields, bare usage aliases, and timestamp fallbacks.
    ...
    static func hasExactCodexPricing(
        _ raw: String,
        modelsDevCatalog: ModelsDevCatalog?,
        pricingResolver: CodexResolver? = nil) -> Bool
    {
        let key = pricingResolver?.normalize(raw) ?? self.normalizeCodexModel(raw)
        guard key != self.codexUnattributedModel else { return false }
        let immutableCatalog = modelsDevCatalog ?? ModelsDevCatalog(providers: [:])
        let lookup = if let pricingResolver {
            pricingResolver.lookup(raw)
        } else {
            self.codexModelsDevLookup(model: raw, catalog: immutableCatalog, cacheRoot: nil)
        }
        if lookup != nil {
            return true
        }
        ...
        modelsDevCacheRoot: URL? = nil) -> Double?
    {
        ...
        return self.claudeCostUSD(model: model, normalizedModel: key, tokens: tokens, pricingDate: pricingDate) {
