    ...
    if (!managementKeyConfigured) {
      ...
      try {
        ...
        if (historyResponse.status !== 200 || latestCompletedResponse.status !== 200) {
          ...
          // The unfiltered history endpoint may include the date-specific
          // bucket. Its counters can legitimately differ from the concurrently
          // fetched response, so let the latter deterministically replace the
          // latest completed day while retaining strict duplicate checks everywhere
          ...
          const rows = [
            ...validatedRows[0].filter(
              (row) =>
                ...
                row.date.trim().slice(0, 10) !== latestCompleted,
            ),
            ...validatedRows[1],
          ];
