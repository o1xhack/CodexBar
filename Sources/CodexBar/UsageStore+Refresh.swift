...
extension UsageStore {
        ...
        coalesceIfRefreshing: Bool = false) async -> ProviderSnapshotPublicationSource?
    {
        ...
        guard await self.providerRefreshCoordinator.wait(for: provider.instanceID, state: request.state) == .completed
        else {
            return nil
        }
        ...
        retryMode: ProviderRefreshRetryMode?) async -> ProviderRefreshRetryMode?
    {
        ...
        if await self.refreshAllTokenAccountsForSnapshotPublicationIfNeeded(
            provider: provider,
    ...
    private func refreshAllTokenAccountsForSnapshotPublicationIfNeeded(
        provider: UsageProvider,
        accounts: [ProviderTokenAccount],
        generation: UInt64) async -> Bool
    {
        guard self.shouldFetchAllTokenAccounts(provider: provider, accounts: accounts) else {
