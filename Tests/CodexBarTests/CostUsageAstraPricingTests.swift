...
struct CostUsageAstraPricingTests {
    ...
    @Test
    func `unpublished bare aliases remain estimated while other provider routes remain unpriced`() throws {
        let environment = try CostUsageTestEnvironment()
        defer { environment.cleanup() }
        ...
        #expect(!CostUsagePricing.hasExactCodexPricing("gpt-6", modelsDevCatalog: ModelsDevCatalog(providers: [:])))
        for model in ["other-provider/gpt-6-astra"] {
            #expect(CostUsagePricing.codexCostUSD(
                model: model,
                inputTokens: 1000,
                cachedInputTokens: 0,
