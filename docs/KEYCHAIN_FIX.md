...
- Keychain is still used for runtime cache entries (for example `com.o1xhack.codexbar.cache`) and Claude OAuth
...
| `ClaudeOAuthCredentials.swift` migrated CodexBar-owned Claude OAuth keychain items | Claude OAuth primary source is Claude CLI keychain service (`Claude Code-credentials`), with CodexBar cache in `com.o1xhack.codexbar.cache` (`oauth.claude`). |
...
`Sources/CodexBar/KeychainMigration.swift` migrates legacy `com.o1xhack.CodexBar` items (for example
...
2. In-memory cache.
3. CodexBar keychain cache (`com.o1xhack.codexbar.cache`, account `oauth.claude`).
...
When CodexBar does not have usable OAuth credentials in its own cache (`com.o1xhack.codexbar.cache` / `oauth.claude`),
...
- Browser-imported Claude session cookies are cached in keychain service `com.o1xhack.codexbar.cache`.
...
- Keychain cache store (`com.o1xhack.codexbar.cache`) also writes with `ThisDeviceOnly`.
...
```bash
defaults read com.o1xhack.codexbar KeychainMigrationV1Completed
```

### Check Claude OAuth keychain cooldown
```bash
defaults read com.o1xhack.codexbar claudeOAuthKeychainDeniedUntil
```

### Inspect keychain-related logs
```bash
log show --predicate 'subsystem == "com.o1xhack.codexbar" && (category == "keychain-migration" || category == "keychain-preflight" || category == "keychain-prompt" || category == "keychain-cache" || category == "claude-usage" || category == "cookie-cache")' --last 10m
```

### Reset migration for local testing
```bash
defaults delete com.o1xhack.codexbar KeychainMigrationV1Completed
