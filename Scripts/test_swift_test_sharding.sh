...
if "types: [closed]" not in workflow:
    raise SystemExit("Final CI must run only after a pull request is closed")
...
if "if: ${{ needs.changes.outputs.macos-tests == 'true' }}" not in job:
    raise SystemExit("swift-test-macos must follow the final-diff path gate")
if not re.search(r"(?m)^\s+shard-index:\s+\[0,\s*1,\s*2,\s*3,\s*4,\s*5\]\s*$", job):
    raise SystemExit("swift-test-macos must run the fork's six shard indexes")
if not re.search(r"(?m)^\s+shard-count:\s+\[6\]\s*$", job):
    raise SystemExit("swift-test-macos shard-count must be [6]")
